...
describe('render > ', () => {
  ...
  describe('리렌더링 테스트 - 변경된 내용만 반영되도록 한다.', () => {
    ...
    test('둘 다 string이라면', () => {
      ...
      expect($root.innerHTML).toBe(`첫 번째`);

      render($root, '두 번째', App);

      expect($root.innerHTML).toBe('두 번째');
    })
    ...
    test('타입이 다르다면', () => {
      ...
      expect($root.innerHTML).toBe(`<span><p>첫 번째 문단</p><p>두 번째 문단</p></span>`);
    })
    ...
    test('중첩자식 추가', () => {
      ...
      expect($root.innerHTML).toBe(`<div id="test-id" class="test-class"><p>첫 번째 문단</p><p><div id="test-id2" class="test-class2"><p>첫 번째 문단</p><p>두 번째 문단</p></div></p></div>`);
    })
